...
/**
 * Default scroll interpolator.
 * Scale and/or opacity effect based on props 'inactiveSlideOpacity' and 'inactiveSlideScale'.
 */
...
/**
 * Default animated styles for carousel items.
 */
export function defaultAnimatedStyles(
  ...
  let animatedOpacity: Record<string, unknown> = {};
  let animatedScale: Record<string, unknown> = {};
...
export function shiftAnimatedStyles(
  ...
  let animatedOpacity: Record<string, unknown> = {};
  let animatedScale: Record<string, unknown> = {};
  let animatedTranslate: Record<string, unknown> = {};
...
/**
 * Stack animation.
 * Imitate a deck/stack of cards.
 *
 * WARNING: The effect had to be visually inverted on Android because this OS
 * doesn't honor the `zIndex` property.
 */
...
/**
 * Stack animated styles for carousel items.
 */
export function stackAnimatedStyles(
  ...
  const resolvedOffset =
    cardOffset !== undefined && cardOffset !== null ? cardOffset : 18;

  const getTranslateFromScale = (cardIndex: number, scale: number): number => {
    const centerFactor = (1 / scale) * cardIndex;
    const centeredPosition = -Math.round(sizeRef * centerFactor);
    const edgeAlignment = Math.round((sizeRef - sizeRef * scale) / 2);
    const offset = Math.round(
      resolvedOffset * Math.abs(cardIndex) / scale
    );
...
/**
 * Tinder scroll interpolator.
 * Imitate the popular Tinder layout.
 *
 * WARNING: The effect had to be visually inverted on Android.
 */
...
/**
 * Tinder animated styles for carousel items.
 */
export function tinderAnimatedStyles(
  ...
  const resolvedOffset =
    cardOffset !== undefined && cardOffset !== null ? cardOffset : 9;

  const getMainTranslateFromScale = (
    cardIndex: number,
    scale: number
  ): number => {
    const centerFactor = (1 / scale) * cardIndex;
  ...
  const getSecondaryTranslateFromScale = (
    cardIndex: number,
    scale: number
  ): number => {
    return Math.round(resolvedOffset * Math.abs(cardIndex) / scale);
  };
